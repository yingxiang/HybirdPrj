...
- (UIView *)containView:(UIView *)view
{
    ...
    [container setBoundsSize:view.frame.size];

#endif
    
    //set view frame
    frame = view.frame;
    frame.origin.x = (container.bounds.size.width - frame.size.width) / 2.0;
    frame.origin.y = (container.bounds.size.height - frame.size.height) / 2.0;
    view.frame = frame;
...
- (UIView *)loadViewAtIndex:(NSInteger)index withContainerView:(UIView *)container
{
    [self pushAnimationState:NO];
    ...
    {
        //get old item view
        UIView *oldItemView = [container.subviews lastObject];
...
{
    //get container view
    UIView *container = [[self itemViewAtIndex:index] superview];
    if (container)
    {
        if (animated)
        {
        ...
        //reload view
        [self loadViewAtIndex:index withContainerView:container];
    }
}
